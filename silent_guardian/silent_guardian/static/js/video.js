// ---------------------------------------------------------------------
// Silent Guardian — Video page logic
...
uploadForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    uploadStatus.textContent =
      `Analyzed ${data.frames_analyzed} sampled frames — peak score ${data.max_score.toFixed(1)}/100.`;
...
function renderTimeline(timeline) {
  ...
  timelineChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        label: "Risk/confidence score (0-100)",
    ...
    options: {
      responsive: true,
      scales: {
        y: { min: 0, max: 100, ticks: { color: "#8fa3b3" }, grid: { color: "#223140" } },
...
function renderIncidents(incidents) {
  if (!incidents.length) {
    uploadResults.innerHTML = `<p class="muted" style="margin-top:14px;">No HIGH/CRITICAL indicators detected.</p>`;
    return;
  }
  const items = incidents.map(inc =>
    `<li>⚠️ Incident #${inc.id} at ${inc.t}s — score ${inc.score}/100 (${inc.classification})</li>`
  ).join("");
  uploadResults.innerHTML =
...
function updateLiveUI(data) {
  ...
  scoreValue.textContent = data.score.toFixed(1) + " / 100";

  liveSeverity.textContent = data.level;
  liveSeverity.className = "severity-tag severity-" + data.level.toLowerCase();

  if (data.incident) {
    liveAlert.style.display = "block";
    liveAlert.innerHTML =
      `🚨 <strong>Incident #${data.incident.id} logged</strong> — score ${data.incident.score}/100 (${data.incident.classification}). ` +
